...
class ECALayer(nn.Module):
    def __init__(self, k_size=3): # Currently hardcoded for simplicity, should be adaptable for future runs
...
class CustomAtrousECAUNet(nn.Module):
    def __init__(self, in_channels=3, classes=4):
        ...
        self.enc3 = AtrousECAConv(128, 256, dilation=2) 
        
        self.pool4 = nn.MaxPool2d(2)
        self.bottleneck = AtrousECAConv(256, 512, dilation=4)
    ...
    def forward(self, x):
        ...
        d1 = self.up1(d2)
        d1 = torch.cat((e1_stem, d1), dim=1) # Catches the 64 channels from the initial "stem"
        d1 = self.dec1(d1)
